...
object RoastPhysicsEngine {
    ...
    fun simulate(input: PhysicsInput): PhysicsOutput {
        ...
        val predictedRor20s =
            (input.currentRor + netEnergy * 0.35).coerceIn(0.0, 30.0)

        val predictedRor30s =
            (input.currentRor + netEnergy * 0.55).coerceIn(0.0, 30.0)

        val summary = when {
            netEnergy > 2.0 ->
                "System energy rising. ROR likely to increase."
            netEnergy < -2.0 ->
                "System energy falling. ROR likely to drop."
            else ->
                "Energy balanced. ROR likely stable."
        }
